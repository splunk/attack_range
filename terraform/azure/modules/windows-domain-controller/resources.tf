...
resource "azurerm_virtual_machine" "dc" {
  ...
  provisioner "local-exec" {
    working_dir = "../../../ansible"
    command = "ansible-playbook -i '${azurerm_public_ip.dc-publicip[count.index].ip_address},' playbooks/windows_dc.yml --extra-vars 'ansible_port=5985 splunk_indexer_ip=${var.config.splunk_server_private_ip} ansible_user=AzureAdmin ansible_password=${var.config.attack_range_password} win_password=${var.config.attack_range_password} splunk_uf_win_url=${var.config.splunk_uf_win_url} win_sysmon_url=${var.config.win_sysmon_url} win_sysmon_template=${var.config.win_sysmon_template} splunk_admin_password=${var.config.attack_range_password} splunk_stream_app=${var.config.splunk_stream_app} s3_bucket_url=${var.config.s3_bucket_url} win_4688_cmd_line=${var.config.win_4688_cmd_line} verbose_win_security_logging=${var.config.verbose_win_security_logging} install_red_team_tools=${var.config.install_red_team_tools} install_aurora_agent=${var.config.install_aurora_agent} aurora_agent_url=${var.config.aurora_agent_url} aurora_agent_license=${var.config.aurora_agent_license} windows_domain_controller_run_badblood=${var.config.windows_domain_controller_run_badblood}'"
  }
