...
resource "aws_instance" "phantom-server" {
  ...
  provisioner "local-exec" {
    working_dir = "../../../ansible/"
    command = "ANSIBLE_HOST_KEY_CHECKING=False ansible-playbook -u ubuntu --private-key ${var.config.private_key_path} -i '${aws_instance.sysmon_linux[0].public_ip},' playbooks/sysmon_linux.yml -e ' phantom_server_private_ip=${var.config.phantom_server_private_ip} sysmon_linux_config=${var.config.sysmon_linux_config}'"
  }
}
